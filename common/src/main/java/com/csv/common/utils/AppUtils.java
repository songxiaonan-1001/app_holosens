    ...
    static boolean isAppForeground() {
        ...
        for (ActivityManager.RunningAppProcessInfo aInfo : info) {
            if (aInfo.importance == ActivityManager.RunningAppProcessInfo.IMPORTANCE_FOREGROUND) {
                if (aInfo.processName.equals(AppUtils.getApp().getPackageName())) {
                    return true;
                }
            }
        }
    ...
    private static void setAnimatorsEnabled() {
        ...
        try {
            ...
            if (sDurationScale == 0f) {
                ...
                Log.i("AppUtils", "setAnimatorsEnabled: Animators are enabled now!");
            }
    ...
    static class ActivityLifecycleImpl implements Application.ActivityLifecycleCallbacks {
        ...
        private void processHideSoftInputOnActivityDestroy(final Activity activity, boolean isSave) {
            if (isSave) {
                ...
                AppUtils.runOnUiThreadDelayed(new Runnable() {
                    @Override
                    public void run() {
